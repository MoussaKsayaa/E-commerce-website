...
let totalDisc = 0;
let currentTotal = 0
let theTotal = document.querySelector('section#add-coupon .cart-totals table .subtotal .price');
let theDisc = document.querySelector('section#add-coupon .cart-totals table .disc .price');
let theCurrentTotal = document.querySelector('section#add-coupon .cart-totals table .total .price');
// nav bar on phone
...
function addItemToLocalStorage() {
  const buyItemBtn = document.querySelector('#single-product .details-side .buy-btn');
  if (buyItemBtn) {
    buyItemBtn.addEventListener('click', function () {
      LS.setItem(`product-${LS.length + 1}`, `{
        "name": "${document.querySelector('#single-product .details-side .product-name').textContent}",
        "price": "${document.querySelector('#single-product .details-side .price').textContent}",
        "img": "${document.querySelector('#single-product .imgs-side .main-img img').src}",
        "quantity": "${document.querySelector('#single-product .details-side .count').value}",
        "size": "${document.querySelector('#single-product .details-side .select-size').value}"
      }`);
      newItemEffect();
    })
  }
...
function newItemEffect() {
  ...
  addItem.innerHTML = '+1';
  document.querySelector('section#header #navbar li.basket').appendChild(addItem);
...
function addItemToCart() {
  ...
  if (LS.length != 0) {
    for (let i = 0; i < LS.length; i++) {
      ...
      tdTotalPrice.innerHTML = `$${data.quantity * data.price.split('$').join('')}`;
...
function saySomething() {
  ...
  row.innerHTML = '<td><div class="warning">there are no items in the cart, <a href="shop.html">Go Shopping..</a></div></td>';
  return row;
}

//* get the products from json file
let productsContainer = document.querySelector('#products1.shop .products');
let productsContainerMain = document.querySelector('#products1.main-page .products');
let productsContainerMain2 = document.querySelector('#products2.main-page .products');
...
function createProductCard(item) {
  ...
  productRate.innerHTML = '<i class="fa-solid fa-star"></i>'.repeat(+item['product-rate']);
...
function changeTheMainProductImg() {
  const mainImg = document.querySelector('#single-product .imgs-side .main-img img');
  const smallImgs = document.querySelectorAll('#single-product .small-imgs-group .col img');
...
function clearThePage() {
  ...
  sectionElement.forEach((section) => {
    if (section.id !== 'header' && section.id !== 'sign-up' && section.id !== 'footer') {
      section.remove();
    }
  })
}
...
function showSimilarProducts(data, item, imgsSide) {
  ...
  let filterdData = data.filter((smallItem) => {
    return (smallItem['product-name'] === item['product-name'] && smallItem['product-brand'] === item['product-brand'] && smallItem['product-price'] === item['product-price'])
  })
  let unique = []
  let randomNum;
...
function showPagination() {
  ...
  document.querySelector('section#products2.featured-products').after(pagination);
}
...
function getTotalPrice() {
  totalPrice = 0;
  currentTotal = 0;
  let allItemsPrice = document.querySelectorAll('section#my-items table tbody tr td.product-totalprice');
